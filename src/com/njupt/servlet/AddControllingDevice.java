...
/**
 * Servlet implementation class AddControllingDevice
 */
@WebServlet("/AddControllingDevice")
public class AddControllingDevice extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AddControllingDevice() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			String result = CloudClient.getInstance().client.addControllingDevice(projectid ,deviceName ,mac ,protocol ,description , localtion, deviceKey);
			if (result.contains("success")) {
				out.println("<script>alert('控制设备添加成功!');location.href='ViewProject?projectid="+projectid+"';</script>");
			}
