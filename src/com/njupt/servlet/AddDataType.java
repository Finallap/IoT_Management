...
/**
 * Servlet implementation class AddDataType
 */
@WebServlet("/AddDataType")
public class AddDataType extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			String result = CloudClient.getInstance().client.addDataType(deviceid, typename, mark, symbol);
			if (result.contains("success")) {
				out.println("<script>alert('数据类型添加成功!');location.href='ViewSensingDevice?deviceid="+deviceid+"';</script>");
			}
